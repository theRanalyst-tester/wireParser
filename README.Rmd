...
#### Capital One
Capital One sends PDF files that are very easy to parse. With the exception of OCR errors (i.e. an "l" instead of a "1"), there should be little to no errors from Capital One parsing. One thing to note is that Capital One's labels seem to be a bit off. For example, "Bene Bank" is sometimes empty whereas "Recv Bank" appears to never be blank. This implies that the true bank of the beneficiary is "Recv Bank". The logic in `parseCapOne` takes this into account and sets the Beneficiary Bank and Intermediary Bank variables accordingly.
...
#### TD Bank
TD Bank sends compliance exactly identical to Capital One. This means that `parseTDBank` is relatively accurate when parsed. Please see the Capital One notes above for things to be aware of.
